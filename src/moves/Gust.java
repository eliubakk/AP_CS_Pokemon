...
public class Gust extends Move
{
	public Gust()
	{
		super("Gust", PokemonGame.Flying, 1, 35, 40, 100, 1, 0);
	}
	...
	public int Attack(SeenPokemon attacker, SeenPokemon defender)
	{
		...
		if(!attacker.getFlinched() && (defender.isProtected()? defender.typeOfProtection().equals("Fly"): true))
		{
			if(!(getAttackType() == 2) && hit <= (getAccuracy() == 0? 100:  getAccuracy() * (attacker.getAccuracyMultiplier()/defender.getEvasiveMultiplier())))
			{
				...
				else
				{
					...
					if(defender.isProtected()? defender.typeOfProtection().equals("Fly"): false)
					{
						Damage *= 2;
					}
				}
	...
	public void secondaryEffect(SeenPokemon Attacker, SeenPokemon Defender, int Damage)//Gust can hit Pokémon during the semi-invulnerable turns of Fly, Bounce, and Sky Drop, and if it does it will deal double damage.
	{
