...
public class Earthquake extends Move
{
	public Earthquake()
	{
		super("Earthquake", PokemonGame.Ground, 0, 10, 100, 100, 1, 0);
	}
	...
	public int Attack(SeenPokemon attacker, SeenPokemon defender)
	{
		...
		if(!attacker.getFlinched() && (defender.isProtected()? defender.typeOfProtection().equals("Dig"): true))
		{
			if(!(getAttackType() == 2) && hit <= (getAccuracy() == 0? 100:  getAccuracy() * (attacker.getAccuracyMultiplier()/defender.getEvasiveMultiplier())))
			{
				if(!critical)
				{
					...
					if(defender.isProtected()? defender.typeOfProtection().equals("Dig"): false)
					{
						Damage *= 2;
					}
				}
	...
	{
		//Does double damage if the defender is in the first stage of dig.
	}
}
