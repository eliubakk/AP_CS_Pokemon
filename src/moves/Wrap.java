...
public class Wrap extends Move
{
	public Wrap()
	{
		super("Wrap", PokemonGame.Normal, 0, 20, 15, 90, 1, 0);
	}
	...
	public void secondaryEffect(SeenPokemon Attacker, SeenPokemon Defender, int Damage)
	{
		...
		{	
			...
			if(chance > 62.5)
			{
				Defender.setTrapped("Wrap", 2);
			}
			else if(chance > 25.0)
			{
				Defender.setTrapped("Wrap", 3);
			}
			else if(chance > 12.5)
			{
				Defender.setTrapped("Wrap", 4);
			}
			else if(chance > 0.0)
			{
				Defender.setTrapped("Wrap", 5);
			}
		}
	}
	public void tertiaryEffect(SeenPokemon Defender)
	{
		if(Defender.getTurnsTrapped() > 1)
		{
			...
			System.out.println(Defender.pokemon.name + " was squeezed by it's WRAP!");
		...
		else if(Defender.getTurnsTrapped() == 0)
		{
			...
			System.out.println(Defender.pokemon.name + " was freed from it's WRAP!");
		}
	}
}
